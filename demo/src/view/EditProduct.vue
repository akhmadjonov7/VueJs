...
export default {
    ...
    try {
      ...
      axios.get('http://94.158.54.194:9092/api/product').then(res => {
        for (let i = 0; i <res.data.length; i++) {
          if (this.productId === res.data[i].id){
            this.nomi = res.data[i].name_uz
            this.narxi = res.data[i].cost
            this.manzili = res.data[i].address
            this.productTypeId = res.data[i].product_type_id
            this.ishlabChiqarilganSana = new Date(res.data[i].created_date).toISOString().substring(0, 16)
          }
        }
  ...
  methods: {
    async SaveProduct() {
      ...
      try {
        const response = await axios.put("http://94.158.54.194:9092/api/product",newProduct)
      }catch (error){
        alert("something went wrong!")
      }
      this.$router.push('/')
